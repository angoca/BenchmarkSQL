 ...
 * 
 * FIXME RDMBS - Adjust for any other one. Add methods for calling stored
 * procedures.
 * 
...
public class jTPCCTData {
	...
	// TODO Refactor to convert this into an abstract class or interface, in order
	// to no modify this when adding new databases.
	...
	private void executeNewOrder(Logger log, jTPCCConnection db) throws Exception {
		...
		/*
		 * When processing the order lines we must select the STOCK rows FOR UPDATE.
		 * This is because we must perform business logic (the juggling with the
		 * S_QUANTITY) here in the application and cannot do that in an atomic UPDATE
		 * statement while getting the original value back at the same time (UPDATE ...
		 * RETURNING may not be vendor neutral). This can lead to possible deadlocks if
		 * two transactions try to lock the same two stock rows in opposite order. To
		 * avoid that we process the order lines in the order of the order of
		...
		try {
			...
			for (int i = 0; i < ol_cnt; i++) {
				...
				if (!rs.next()) {
					...
					/*
					 * 1% of NEW_ORDER transactions use an unused item in the last line to simulate
					 * user entry errors. Make sure this is precisely that case.
					 */
					if (transRbk && (newOrder.ol_i_id[seq] < 1 || newOrder.ol_i_id[seq] > 100000)) {
						/*
						 * Clause 2.4.2.3 mandates that the entire transaction profile up to here must
						 * be executed before we can roll back, except for retrieving the missing STOCK
						 * row and inserting this ORDER_LINE row. Note that we haven't updated STOCK
						 * rows or inserted any ORDER_LINE rows so far, we only batched them up. So we
						 * must do that now in order to satisfy 2.4.2.3.
						 */
	...
	private void executeDelivery(Logger log, jTPCCConnection db) {
		...
		/*
		 * The DELIVERY transaction is different from all the others. The foreground
		 * transaction, experienced by the user, does not perform any interaction with
		 * the database. It only queues a request to perform such a transaction in the
		 * background (DeliveryBG). We store that TData object in the delivery part for
		 * the caller to pick up and queue/execute.
		 */
	...
	private class DeliveryData {
		...
		/*
		 * executeDelivery() will store the background request here for the caller to
		 * pick up and process as needed.
		 */
	...
	private void generateDeliveryBG(int w_id, long due, String ol_delivery_d, jTPCCTData parent) {
		/*
		 * The DELIVERY_BG part is created as a result of executing the foreground part
		 * of the DELIVERY transaction. Because of that it inherits certain information
		 * from it.
		 */
	...
	private void executeDeliveryBG(Logger log, jTPCCConnection db) throws Exception {
		...
		try {
			for (d_id = 1; d_id <= 10; d_id++) {
				...
				/*
				 * Try to find the oldest undelivered order for this DISTRICT. There may not be
				 * one, which is a case that needs to be reportd.
				 */
				while (o_id < 0) {
					...
					if (rc == 0) {
						/*
						 * Failed to delete the NEW_ORDER row. This is not an error since for
						 * concurrency reasons we did not select FOR UPDATE above. It is possible that
						 * another, concurrent DELIVERY_BG transaction just deleted this row and is
						 * working on it now. We simply got back and try to get the next one. This logic
						 * only works in READ_COMMITTED isolation level and will cause SQLExceptions in
				...
				/*
				 * We found out oldest undelivered order for this DISTRICT and the NEW_ORDER
				 * line has been deleted. Process the rest of the DELIVERY_BG.
				 */
